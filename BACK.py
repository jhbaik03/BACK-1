...
# Frame에 내용 삽입
        ...
        class Champion:
            def __init__(self, parent, image_path):
                ...
                inframe = tkinter.Label(self.frame_champion, image=self.img, text="Aatrox", border=0)
                inframe.bind("<Button-1>", callback_champion_click)
...
# 스크롤바에도 Canvas 위젯 연결
    ...
    # 입력값을 포함하는 Frame만 저장하기
            filtered_text = []
            for frame in frame_champions:
                if text.lower() in frame['text'].lower():
                    filtered_text.append(frame['text'].lower())
    ...
    # print(i)
                ...
                frame_champions[i] = tkinter.LabelFrame(frame_champions_line[line_num], width = frame_champions_width, height= frame_champions_height, relief="solid", bg="white", text=str(filtered_text[i]))
